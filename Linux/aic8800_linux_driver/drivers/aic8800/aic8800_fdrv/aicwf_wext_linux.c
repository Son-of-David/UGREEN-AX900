...
static int aicwf_set_mode(struct net_device *dev,
                          struct iw_request_info *info,
                          union iwreq_data *wrqu,
                          char *extra)
{
    struct wireless_dev *wdev = dev ? dev->ieee80211_ptr : NULL;
    enum nl80211_iftype want;

    if (!wdev)
        return -EOPNOTSUPP;

    /* Map WEXT mode -> nl80211 iftype */
    switch (wrqu->mode) {
    case IW_MODE_MONITOR:
        want = NL80211_IFTYPE_MONITOR;
        break;
    case IW_MODE_INFRA:
        want = NL80211_IFTYPE_STATION;
        break;
    default:
        return -EOPNOTSUPP;
    }
    ...
    /*
     * IMPORTANT:
     * Do NOT call into rwnx_cfg80211_change_iface() here.
     * That code path has been observed to NULL-deref via WEXT.
     *
     * We only report success if we're already in the requested mode.
     */
    return (wdev->iftype == want) ? 0 : -EOPNOTSUPP;
}
