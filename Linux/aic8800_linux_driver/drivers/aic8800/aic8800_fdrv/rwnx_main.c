...
/* Timer shutdown helper for newer kernels (6.x) */
static inline void aic_timer_shutdown_sync(struct timer_list *t)
{
#if defined(timer_shutdown_sync)
    timer_shutdown_sync(t);
#elif defined(timer_delete_sync)
    timer_delete_sync(t);
#else
    /*
     * Extremely old kernels only. If you truly need them, you'll have to
     * include proper timer prototypes or add compat code.
     * For Kali 6.17 this path won't be used.
     */
    (void)t;
#endif
}
   ...
   // printk("del_iface: %p\n",wdev);
	...
	if (rwnx_vif == rwnx_hw->p2p_dev_vif) {
		if (timer_pending(&rwnx_hw->p2p_alive_timer)) {
			aic_timer_shutdown_sync(&rwnx_hw->p2p_alive_timer);
		}
	}
#endif
...
int aicwf_cfg80211_change_iface(struct wiphy *wiphy,
                               struct net_device *dev,
                               enum nl80211_iftype type,
                               struct vif_params *params)
{
    return rwnx_cfg80211_change_iface(wiphy, dev, type, params);
}
    ...
    if (!list_empty(&rwnx_hw->defrag_list)) {
        list_for_each_entry(defrag_ctrl, &rwnx_hw->defrag_list, list) {
            ...
            if (timer_pending(&defrag_ctrl->defrag_timer))
                aic_timer_shutdown_sync(&defrag_ctrl->defrag_timer);
